...
class DBSCAN:
    ...
    Methods:
        ...
        euclidean_dist(v1, v2): Calculates the Euclidean distance between two points `v1` and `v2`.
    ...
    def _get_neighbors(self, p_idx, X):
        ...
        for idx, p in enumerate(X):
            if p_idx == idx:
                continue
            if self.euclidean_dist(X[idx], X[p_idx]) < self.eps:
    ...
    def fit(self, X):
        ...
        Parameters:
            X (numpy.ndarray): The data to fit the DBSCAN algorithm to.
